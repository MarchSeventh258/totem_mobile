...
public class SelectImpl implements edu.whu.tmdb.query.operations.Select {
    ...
    @Override
    /**
     * 该查询语句为join语句，且指定连接元组，用于join代理类的实现
     * @param t 指定的连接元组
     */
    public SelectResult joinselect(Object stmt,TupleList tupleList) throws TMDBException, IOException {
        SelectBody selectBody = null;
        if (stmt.getClass().getSimpleName().equals("Select")) {         // 不带子查询的类型转换
            selectBody = ((net.sf.jsqlparser.statement.select.Select)stmt).getSelectBody();
        }else if (stmt.getClass().getSimpleName().equals("SubSelect")) {// 带子查询的类型转换
        ...
        assert selectBody != null;
        if ((selectBody.getClass().getSimpleName().equals("SetOperationList"))) {
            // 带union，except关键字的查询,应为join操作，抛错处理
            throw new TMDBException(ErrorList.TYPE_IS_NOT_SUPPORTED,"SetOperation");
        }else {     // 简单查询
            res = joinplainSelect(selectBody,tupleList);
        }
    ...
    public SelectResult joinplainSelect(SelectBody stmt,TupleList tupleList) throws TMDBException, IOException {
        // Values 也是一种plainSelect，如果是values，由values方法处理
        if(stmt.getClass().getSimpleName().equals("ValuesStatement")) {
            return values((ValuesStatement) stmt);
        }
        ...
        // 以下是常规plainselect逻辑：from->where->select
        // 1.调用joinfrom方法获取相关的所有元数据
        SelectResult selectResult = joinfrom(plainSelect,tupleList);
        // 2.调用where对元数据进行进行筛选
        if (plainSelect.getWhere() != null){
            Where where = new Where();
            selectResult = where.where(plainSelect, selectResult);
    ...
    private SelectResult limit(int limit, SelectResult selectResult) {
        // 获取原始的元组数据列表
        TupleList originalTpl = selectResult.getTpl();
        
        // 获取截断后的元组数据列表
        TupleList limitedTpl = new TupleList();
        List<Tuple> limitedLst = new ArrayList<>();
        limitedTpl.tuplenum = limit < originalTpl.tuplenum ? limit : originalTpl.tuplenum;
        for (int i = 0; i < limitedTpl.tuplenum; i++) {
            limitedLst.add(originalTpl.tuplelist.get(i));
        }
        limitedTpl.tuplelist = limitedLst;
        
        // 更新原始的 SelectResult 对象的元组数据列表
        selectResult.setTpl(limitedTpl);
...
//            这里是针对selectItem是表达式的判定，a或者a+2或者a+b*c都会被认定为表达式
        ...
        // 创建一个新的 HashMap<Object, ArrayList<Integer>> 类型的 groupMap
        HashMap<Object, ArrayList<Integer>> groupMap = new HashMap<>();

        // 遍历 resultMap
        for (HashMap.Entry<Object, ArrayList<Tuple>> entry : resultMap.entrySet()) {
    ...
    // 给定一个分组依据和它相关的原始tuple列表，返回聚合后的tuple
    public Tuple aggOne(Object groupObj, TupleList tpl, ArrayList<String> funcNames, ArrayList<Integer> pIndexs) {
        Tuple res = new Tuple();
        res.tuple = new Object[pIndexs.size() + 1];
        res.tuple[0] = groupObj;
        for (int idx = 0; idx < funcNames.size(); idx++) {
            int pi = pIndexs.get(idx);
            String func = funcNames.get(idx);
            List<Double> temp = new ArrayList<Double>();
            for (Tuple t : tpl.tuplelist) {
                if(t.tuple[pi]!=null) {
                    temp.add(Double.parseDouble((String) t.tuple[pi]));
                }
            }
            double d=solveList(func, temp);
            res.tuple[idx + 1] = d;
        }
        return res;
    }
    ...
    public SelectResult projection(PlainSelect plainSelect, SelectResult entireResult) throws TMDBException {
        ...
        if(entireResult.getisJoin()) {projectResult.setisJoin(entireResult.getJoinColumnNames());}
    ...
    private void projectSelectExpression(SelectItem item, SelectResult entireResult,
        ...
        // 1.attrName赋值
       // 1. 获取原始列名
       String fullOriginalName = selectItem.toString();
       String originalName = fullOriginalName.contains(" ") ? fullOriginalName.substring(0, fullOriginalName.indexOf(" ")) : fullOriginalName; 
                    
        // 2. 获取别名（如果存在）
        String aliasName = selectItem.getAlias() != null ? selectItem.getAlias().getName() : null;

        // 3. 赋值 attrName：仍然使用别名（如果有的话）
        projectResult.getAttrname()[indexInResult] = aliasName != null ? aliasName : originalName;

        // 4. 赋值 alias：存储原始列名（如果有别名的话）
        projectResult.getAlias()[indexInResult] = originalName ;
        ...
        for (int  i = 0;  i< resTupleList.tuplelist.size(); i++) {
            // 找到表达式对应属性在原元组对应的下标后，将其值赋值给对应的元组 byjinx
            ...
            resTupleList.tuplelist.get(i).tupleIds[indexInResult] = entireResult.getTpl().tuplelist.get(i).tupleIds[indexInEntireResult];
            projectResult.getAttrid()[indexInResult] =  entireResult.getTpl().tuplelist.get(i).tupleIds[indexInEntireResult];
        }
        // 4.剩余属性赋值
        projectResult.getType()[indexInResult] = entireResult.getType()[indexInEntireResult];
        projectResult.getAttrid()[indexInResult] = entireResult.getAttrid()[indexInEntireResult];
    ...
    public SelectResult from(PlainSelect plainSelect) throws TMDBException {
        ...
        ArrayList<AttributeTableItem> attributeTableItemList = memConnect.copyAttributeTableList(fromItem);     // 获取class item信息，对应于select输出列表的表头
        SelectResult selectResult = getSelectResult(attributeTableItemList, tupleList);
        ...
        if(!(plainSelect.getJoins() == null)){
            ...
            for (Join join:plainSelect.getJoins()) {
                // 获取当前join表的的一些元祖
                ArrayList<AttributeTableItem> tempAttributeTableItem = memConnect.copyAttributeTableList(join.getRightItem());
                TupleList tempTupleList = memConnect.getTupleList(join.getRightItem());
                SelectResult tempSelectResult = getSelectResult(tempAttributeTableItem, tempTupleList);
                // 将本来的TupleList和当前操作的join的tuplelist根据join的形式进行组合
                tupleList = join(selectResult,tempSelectResult,join);
                // 把classTableItem进行合并
                attributeTableItemList.addAll(tempAttributeTableItem);
                // 根据join后的tuple和合并后的classtableItemlist生成selectResult
                selectResult=getSelectResult(attributeTableItemList, tupleList);
                //用于设置join代理规则
                ...
                if(!expressionLinkedList.isEmpty()){
                    ...
                    joinColumnNames.add(leftExpression.getFullyQualifiedName());
                    //获取等于表达式的右边
                    Column rightExpression=(Column) equals.getRightExpression();
                    joinColumnNames.add(rightExpression.getFullyQualifiedName());
                }
            }
            selectResult.setisJoin(joinColumnNames);
        }
        return selectResult;
    }
     ...
     * @param plainSelect 平凡查询语句
     * @param t 指定连接元组
     * @return join查询语句中涉及的所有元数据
     */
    public SelectResult joinfrom(PlainSelect plainSelect,TupleList inserttupleList) throws TMDBException {
        FromItem fromItem = plainSelect.getFromItem();              // 获取plainSelect的表（多表查询时取第一个table）
        if (fromItem == null) { throw new TMDBException(ErrorList.MISSING_FROM_CLAUSE ); }
        int classId = memConnect.getClassId(((Table) fromItem).getName());
        TupleList tupleList = null;
        if(inserttupleList.tuplenum!=0) {
            if(classId==inserttupleList.tuplelist.get(0).classId){ 
                tupleList=inserttupleList;//如果指定连接元组是该表的，则无需取所有元组，拿这些元组即可
            }else{
                tupleList = memConnect.getTupleList(fromItem);    // 获取from后面table的所有元组
            }
        }
        else {
        ...
        if(!(plainSelect.getJoins() == null)){
            for (Join join:plainSelect.getJoins()) {
                ...
                ArrayList<AttributeTableItem> tempAttributeTableItem = memConnect.copyAttributeTableList(rightItem);
                int rightclassId = memConnect.getClassId(((Table) rightItem).getName());
                TupleList tempTupleList = new TupleList();
                if(inserttupleList.tuplenum!=0) {
                    if(rightclassId==inserttupleList.tuplelist.get(0).classId){ 
                        tempTupleList=inserttupleList;//如果指定连接元组是该表的，则无需取所有元组，拿这些元组即可
                    }else{
                        tempTupleList = memConnect.getTupleList(rightItem);    // 获取from后面table的所有元组
                    }
                }
                else {
                    throw new TMDBException();
    ...
    public SelectResult deputySelect(PlainSelect plainSelect) throws TMDBException {
        ...
        ArrayList<AttributeTableItem> attributeTableItemList=this.getSelectItem(fromItem,selectColumnList);
        return getSelectResult(attributeTableItemList,tupleList);
    }
    ...
    public SelectResult Operate(SelectResult selectResult1, SelectResult selectResult2, SetOperation setOperation) throws TMDBException {
        ...
        String a=setOperation.toString();
        switch (a){

            case "UNION":       return union(selectResult1, selectResult2);
            case "UNION ALL":    return unionall(selectResult1, selectResult2);
    ...
    public SelectResult union(SelectResult selectResult1,SelectResult selectResult2){
        ...
        //byjinx HashSet<Tuple> set=new HashSet<>();
        //将两个tuplelist都加入最终的结果集合里
        int classid1=0;
        int classid2=0;
        for(Tuple tuple:selectResult1.getTpl().tuplelist){
            set.add(tuple);
            classid1=tuple.classId;
        }
        for(Tuple tuple:selectResult2.getTpl().tuplelist){
            set.add(tuple);
            classid2=tuple.classId;
        }
        Map<AbstractMap.SimpleEntry<Integer, Integer>, String> aliasMap = new HashMap<>();
            // 处理 selectResult1
        for (int i = 0; i < selectResult1.getAlias().length; i++) {
             // 获取 classId（假设用 className 的哈希值表示）
             String reslt=selectResult1.getAlias()[i];
            aliasMap.put(new AbstractMap.SimpleEntry<>(classid1, i), reslt);
        }

        // 处理 selectResult2
        for (int i = 0; i < selectResult2.getAlias().length; i++) {
            String reslt=selectResult2.getAlias()[i];
            aliasMap.put(new AbstractMap.SimpleEntry<>(classid2, i), reslt);
        }
        List<Tuple> res = new ArrayList<Tuple>(set);
        tupleList.tuplelist=res;
        tupleList.tuplenum=set.size();
        //selectResult1.setClassName(selectResult1.getAlias());
        selectResult1.setTpl(tupleList);
        selectResult1.setisUnion();
        selectResult1.aliasMapping=aliasMap;
        return selectResult1;
    }
    public SelectResult unionall(SelectResult selectResult1,SelectResult selectResult2){
        TupleList tupleList=new TupleList();
        ArrayList<Tuple> set = new ArrayList<>();
        //byjinx HashSet<Tuple> set=new HashSet<>();
        //将两个tuplelist都加入最终的结果集合里
    ...
    //针对values的处理
    public SelectResult values(ValuesStatement valuesStatement) throws TMDBException{
        ...
        ArrayList<AttributeTableItem> attributeTableItemArrayList=new ArrayList<>();
        //构建classtableItemlist，返回selctResult需要
        for(int i=0;i<tupleList.tuplelist.get(0).tuple.length;i++){
            AttributeTableItem attributeTableItem=new AttributeTableItem(-1, i, "attr"+i, tupleList.tuplelist.get(0).tuple.getClass().getSimpleName(), "");
    ...
    public TupleList join(SelectResult left,SelectResult right,Join join) throws TMDBException {
        ...
        if(!expressionLinkedList.isEmpty()){
            ...
            else { // 没有说明什么join，默认就是inner join
                leftTupleList=naturalJoin(leftTupleList,rightTupleList,leftIndex,rightIndex);
            }
        }
        else{
    ...
    // TODO-Task10 support null in left and right join
     ...
     * @param attributeTableItemList class item，对应查询结果的表头
     * @param tupleList 元组列表，对应查询结果的元数据
     * @return 包含所有查询元组的selectResult类
     */
    public SelectResult getSelectResult(ArrayList<AttributeTableItem> attributeTableItemList, TupleList tupleList) throws TMDBException{
        SelectResult selectResult = new SelectResult(attributeTableItemList.size());
        for(int i = 0; i < attributeTableItemList.size();i++){
            //TODO-opt classname
            selectResult.getClassName()[i] = memConnect.getClassName(attributeTableItemList.get(i).classid);
            selectResult.getAlias()[i] = attributeTableItemList.get(i).alias;
            selectResult.getAttrid()[i] = attributeTableItemList.get(i).attrid;
            selectResult.getAttrname()[i] = attributeTableItemList.get(i).attrname;
            selectResult.getType()[i] = attributeTableItemList.get(i).attrtype;
        }
        selectResult.setTpl(tupleList);
        return selectResult;
    }

    public SelectResult getValueResult(ArrayList<AttributeTableItem> attributeTableItemList, TupleList tupleList) throws TMDBException{
        SelectResult selectResult = new SelectResult(attributeTableItemList.size());
        for(int i = 0; i < attributeTableItemList.size();i++){
            //TODO-opt classname
            selectResult.getClassName()[i] = "";
            selectResult.getAlias()[i] = attributeTableItemList.get(i).alias;
            selectResult.getAttrid()[i] = attributeTableItemList.get(i).attrid;
            selectResult.getAttrname()[i] = attributeTableItemList.get(i).attrname;
            selectResult.getType()[i] = attributeTableItemList.get(i).attrtype;
    ...
    public ArrayList<AttributeTableItem> getSelectItem(FromItem fromItem, List<Column> columnList) throws TMDBException{
        // 从class表中提取将要获取的元素。
        int classId = memConnect.getClassId(fromItem.toString());
        ArrayList<AttributeTableItem> elicitAttrItemList=new ArrayList<>();
