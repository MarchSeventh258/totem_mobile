...
//                    log.WriteLog(id,k,op,s);
 ...
 //                   log.WriteLog(id,k,op,s);
    ...
    public SelectResult joinquery(String k, int op, Statement stmt, TupleList inserttupleList) {
        SelectResult selectResult = null;
        try {
            // 获取生成语法树的类型，用于进一步判断
            String sqlType = stmt.getClass().getSimpleName();
            if(Objects.equals(sqlType,"Select")) {//用于源类1个对象和其它源类的连接操作
                Select joinselect = new SelectImpl();
                selectResult = joinselect.joinselect(stmt,inserttupleList);
            }
            else {
                throw new TMDBException(ErrorList.TYPE_IS_NOT_SUPPORTED,sqlType);
            }
        } catch (IOException e) {
            logger.error(e.getMessage(),e);
        } catch (TMDBException e) {
