...
public class MemManager {
    ...
    public static AttributeTable attributeTable = new AttributeTable();
    ...
    public static DeputyRuleTable deputyRuleTable = new DeputyRuleTable();
...
//        }
    ...
    public void saveClassTable() throws IOException {
        ...
        for(ClassTableItem item : this.classTable.classTableList){
            ...
            // 存classtype
            writeAccess.write(Constant.INT_TO_BYTES(item.classtype));
    ...
    // AttributeTableItem有以下属性
    // int      classid
    // int      attrid
    // String   attrname
    // String   attrtype
    // String   alias
    public void saveAttributeTable() throws IOException {
        File f = new File(Constant.SYSTEM_TABLE_DIR + "at");
        if (!f.exists()) {
            f.createNewFile();
        }
        BufferedOutputStream writeAccess = new BufferedOutputStream(new FileOutputStream(f));
        ...
        // 存各个ClassTableItem
        for(AttributeTableItem item : this.attributeTable.attribuleTableList){
            // 存classid
            writeAccess.write(Constant.INT_TO_BYTES(item.classid));
            // 存attrid
            writeAccess.write(Constant.INT_TO_BYTES(item.attrid));
            // 存isVirtual
            writeAccess.write(Constant.INT_TO_BYTES(item.isVirtual));
    ...
    public void loadAttributeTable() throws IOException {
        File f = new File(Constant.SYSTEM_TABLE_DIR + "at");
        if (!f.exists()) {
            return;
        }
        ...
        while(cur < fileSize){
            // 读各个ClassTableItem
            AttributeTableItem item = new AttributeTableItem();
            // 读classid
            item.classid = raf.readInt();
            cur += Integer.BYTES;
            // 读attrid
            ...
            this.attributeTable.attribuleTableList.add(item);
        }
    }
    ...
    public void saveDeputyTable() throws IOException {
        ...
        for(DeputyTableItem item : this.deputyTable.deputyTableList){
            ...
            writeAccess.write(Constant.INT_TO_BYTES(item.deputyid));
            // 存deputyid
            writeAccess.write(Constant.INT_TO_BYTES(item.ruleid));
        }
        writeAccess.flush();
        writeAccess.close();
    }
    ...
    public void loadDeputyTable() throws IOException {
        ...
        while(cur < fileSize){
            ...
            // 读ruleid
            item.ruleid = raf.readInt();
            cur += Integer.BYTES;
            this.deputyTable.deputyTableList.add(item);
        }
    }

    // DeputyRuleTableItem 有以下属性
    // int maxruleid
    // int ruleid
    // String[] deputyrule
    public void saveDeputyRuleTable() throws IOException {
        File f = new File(Constant.SYSTEM_TABLE_DIR + "dt");
        if(!f.exists()) {
            f.createNewFile();
        }
        BufferedOutputStream writeAccess = new BufferedOutputStream(new FileOutputStream(f));
        // 存maxClassId
        writeAccess.write(Constant.INT_TO_BYTES(this.deputyRuleTable.maxruleid));
        for(DeputyRuleTableItem item : this.deputyRuleTable.deputyRuleTableList){
            // 存ruleid
            writeAccess.write(Constant.INT_TO_BYTES(item.ruleid));
    ...
    public void loadDeputyRuleTable() throws IOException {
        File f = new File(Constant.SYSTEM_TABLE_DIR + "dt");
        if(!f.exists()) { return; }
        ...
        long cur = 0L;
        // 先读maxRuleId
        this.deputyRuleTable.maxruleid = raf.readInt();
        cur += Integer.BYTES;
        while(cur < fileSize){
            DeputyRuleTableItem item = new DeputyRuleTableItem();
            ...
            this.deputyRuleTable.deputyRuleTableList.add(item);
        }
    }
    ...
    public void loadSwitchingTable() throws IOException {
        ...
        while (cur < fileSize) {
            ...
            // Read oriAttr (length + content)
            int oriAttrLength = raf.readInt();
            cur += Integer.BYTES;
            byte[] oriAttrBuffer = new byte[oriAttrLength];
            raf.read(oriAttrBuffer);
            cur += oriAttrLength;
            item.oriAttr = new String(oriAttrBuffer);
            ...
            // Read deputyAttr (length + content)
            int deputyAttrLength = raf.readInt();
            cur += Integer.BYTES;
            byte[] deputyAttrBuffer = new byte[deputyAttrLength];
            raf.read(deputyAttrBuffer);
            cur += deputyAttrLength;
            item.deputyAttr = new String(deputyAttrBuffer);
    
            // Read rule (length + content)
            int ruleLength = raf.readInt();
            cur += Integer.BYTES;
            byte[] ruleBuffer = new byte[ruleLength];
            raf.read(ruleBuffer);
            cur += ruleLength;
            item.rule = new String(ruleBuffer);
